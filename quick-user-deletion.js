...
class UserDeletionTool {
  ...
  async deleteUserByEmail(email) {
    try {
      ...
      // 1. 查找用户ID
      const findResponse = await fetch(`${this.baseUrl}/api/admin/find-user?email=${encodeURIComponent(email)}`, {
        credentials: 'include',
      });
      ...
      const previewResponse = await fetch(`${this.baseUrl}/api/admin/delete-user?userId=${userId}`, {
        credentials: 'include',
      });
      ...
      console.log(`    - 头像制作历史: ${preview.dataToDelete.profilePictureHistory} 条`);
      console.log(`    - 贴纸生成历史: ${preview.dataToDelete.stickerHistory} 条`);
      console.log(`    - 产品拍摄历史: ${preview.dataToDelete.productshotHistory} 条`);
      console.log(`    - 水印移除历史: ${preview.dataToDelete.watermarkHistory} 条`);
      console.log(`    - 积分交易记录: ${preview.dataToDelete.creditsTransactions} 条`);
      ...
      const deleteResponse = await fetch(`${this.baseUrl}/api/admin/delete-user?userId=${userId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
  ...
  async verifyDeletion(email) {
    try {
      const response = await fetch(`${this.baseUrl}/api/admin/find-user?email=${encodeURIComponent(email)}`, {
        credentials: 'include',
      });
...
window.batchDeleteUsers = (emails) => window.userDeletionTool.batchDeleteUsers(emails);
window.verifyUserDeleted = (email) => window.userDeletionTool.verifyDeletion(email);
