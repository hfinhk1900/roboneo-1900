...
async function convertToWebP(inputPath) {
  ...
  try {
    ...
    console.log(
      `   Size: ${(fileSize / 1024).toFixed(1)}KB → ${(webpSize / 1024).toFixed(1)}KB (${reduction}% reduction)`
    );
...
async function main() {
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      console.log(
        `   Total size before: ${(totalOriginal / 1024 / 1024).toFixed(2)}MB`
      );
      console.log(
        `   Total size after: ${(totalWebP / 1024 / 1024).toFixed(2)}MB`
      );
