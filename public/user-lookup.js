  ...
  try {
    ...
    if (response.ok) {
      ...
      data.users.forEach((user, index) => {
        ...
        console.log(
          `   订阅状态: ${user.subscriptionSummary.hasActiveSubscription ? '有活跃订阅' : '无活跃订阅'}`
        );
        console.log(`   支付记录数: ${user.subscriptionSummary.totalPayments}`);
        if (user.subscriptionSummary.latestSubscriptionId) {
          console.log(
            `   最新订阅ID: ${user.subscriptionSummary.latestSubscriptionId}`
          );
        }
        console.log('---');
      });
  ...
  try {
    ...
    if (paymentsResponse.ok) {
      ...
      paymentsData.payments.forEach((payment, index) => {
        console.log(
          `${index + 1}. 状态: ${payment.status} | 类型: ${payment.type} | 创建: ${payment.createdAt}`
        );
