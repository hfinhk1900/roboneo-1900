  ...
  try {
    ...
    if (!target.host) {
      try {
        ...
        const etag = '"' + createHash('sha1').update(Buffer.from(buf)).digest('hex') + '"';
        const ifNone = req.headers.get('if-none-match');
        if (ifNone && ifNone === etag) {
          return new NextResponse(null, {
            status: 304,
            headers: { ETag: etag, 'Cache-Control': 'public, max-age=3600, stale-while-revalidate=60' },
          });
        }
        ...
        return NextResponse.json({ error: 'Avatar fetch error' }, { status: 502 });
      }
    }

    if (!allowedHosts.has(target.host)) {
      return NextResponse.json({ error: 'Avatar host not allowed' }, { status: 403 });
    }
    ...
    if (!resp.ok) {
      return NextResponse.json({ error: 'Avatar upstream error' }, { status: 502 });
    }
