...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(
      `📂 Found ${assetIds.length} assets to delete from ${historyRecords.length} Sticker history items`
    );
    ...
    if (assetIds.length > 0) {
      ...
      console.log(
        '📊 Sticker asset deletion summary:',
    ...
    console.log(
      `✅ Sticker batch deletion completed: ${foundIds.length} history items deleted`
    );
