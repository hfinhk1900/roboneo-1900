...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(`🗑️ Batch deleting ${ids.length} ProductShot history items...`);
    ...
    const historyRecords = await db
      ...
      .where(
        and(
          inArray(productshotHistory.id, ids),
    ...
    console.log(`📂 Found ${assetIds.length} assets to delete from ${historyRecords.length} ProductShot history items`);

    // 3. 批量删除资产文件
    let assetDeletionResult = null;
    if (assetIds.length > 0) {
      assetDeletionResult = await deleteAssets(assetIds, session.user.id);
      console.log(`📊 ProductShot asset deletion summary:`, assetDeletionResult.summary);
    }
    ...
    await db
      ...
      .where(
        and(
          inArray(productshotHistory.id, foundIds),
          eq(productshotHistory.userId, session.user.id)
        )
      );

    console.log(`✅ ProductShot batch deletion completed: ${foundIds.length} history items deleted`);
    ...
    console.error('Error batch deleting productshot history:', error);
    return NextResponse.json(
