...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(
      `📂 Found ${assetIds.length} assets to delete from ${historyRecords.length} ProductShot history items`
    );
    ...
    if (assetIds.length > 0) {
      ...
      console.log(
        '📊 ProductShot asset deletion summary:',
    ...
    console.log(
      `✅ ProductShot batch deletion completed: ${foundIds.length} history items deleted`
    );
