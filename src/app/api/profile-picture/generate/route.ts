...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `💳 User ${session.user.id} has ${currentCredits} credits, proceeding with profile picture generation...`
    );
    ...
    console.log('🤖 Profile picture generation prompt:', {
    ...
    if (deductResult?.data?.success) {
      console.log(
        `💰 Deducted ${CREDITS_PER_IMAGE} credits for profile picture generation. Remaining: ${deductResult.data.data?.remainingCredits}`
      );
    } else {
      console.warn(
        '⚠️ Failed to deduct credits, but profile picture was generated successfully'
      );
    }
    ...
    const fileName = result.resultUrl.split('/').pop() || 'profile-picture.png';
    ...
    console.warn('Profile picture generation error:', error);
    ...
    let statusCode = 500;
    let userMessage = 'Profile picture generation failed';
...
export async function GET() {
  return NextResponse.json({
    service: 'profile-picture-generator',
    status: 'available',
    model: 'black-forest-labs/FLUX.1-Kontext-dev',
    provider: 'SiliconFlow',
    credits_per_image: CREDITS_PER_IMAGE,
    styles: ['professional', 'business', 'clean', 'monochrome'],
  });
}
