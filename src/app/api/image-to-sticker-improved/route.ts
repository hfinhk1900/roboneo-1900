...
async function preprocessToSquareRGBA(inputBuffer: Buffer): Promise<{
  ...
  try {
    ...
    console.log('📊 [IMPROVED] Original image:', {
    ...
    console.log('✅ [IMPROVED] Preprocessing complete:', {
 ...
 */
async function gptStyleTransferImproved(
  base64Data: string,
  prompt: string,
  style: string,
  apiKey: string
  ...
  try {
    console.log(
      `🎨 [IMPROVED] Calling GPT Image 1 for ${style} style transfer with MEDIUM quality...`
    );
    console.log(
      `📝 [IMPROVED] Using enhanced prompt: "${prompt.substring(0, 100)}..."`
    );
    console.log(
      `💰 [IMPROVED] Cost optimized: ~$0.042 per image (vs $0.169 high quality)`
    );
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      console.log(
        '🧪 [IMPROVED] Development mode: returning mock sticker response'
      );
    ...
    console.log(
      `💳 [IMPROVED] User ${session.user.id} has ${creditsCheck.data.data.currentCredits} credits, proceeding with generation...`
    );
    ...
    console.log(
      `✅ [IMPROVED] File validation passed: ${imageFile.name} (${Math.round(imageFile.size / 1024)}KB, ${imageFile.type})`
    );
    ...
    const stickerBase64 = await gptStyleTransferImproved(
    ...
    if (!stickerBase64) {
      return NextResponse.json(
        { error: 'Failed to generate sticker with improved GPT Image 1' },
        { status: 500 }
      );
    }
    ...
    if (deductResult?.data?.success) {
      console.log(
        `💰 [IMPROVED] Deducted ${CREDITS_PER_IMAGE} credits. Remaining: ${deductResult.data.data?.remainingCredits}`
      );
    } else {
      console.warn(
        '⚠️ [IMPROVED] Failed to deduct credits, but sticker was generated successfully'
      );
    }
    ...
    console.log(
      `🎉 [IMPROVED] Enhanced sticker generation complete! Total time: ${Math.round(elapsed / 1000)}s`
    );
