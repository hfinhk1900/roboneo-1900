...
export function analyzeProductWithHeuristics(
  ...
  return {
    ...
    reasoning: reasons.length > 0 ? reasons.join('; ') : 'Default heuristic analysis',
    metadata,
 ...
 * 分析文件名中的关键词
 */
function analyzeFilename(filename: string): { category?: 'small' | 'medium' | 'large'; confidence: number; reason?: string } {
  const name = filename.toLowerCase();
  
  // 小型产品关键词
  const smallKeywords = ['ring', 'watch', 'phone', 'perfume', 'cosmetic', 'jewelry', 'lipstick', 'earring', 'necklace', 'bracelet'];
  if (smallKeywords.some(keyword => name.includes(keyword))) {
    return { category: 'small', confidence: 0.8, reason: `filename contains small product keyword` };
  }
  
  // 大型产品关键词  
  const largeKeywords = ['furniture', 'chair', 'table', 'sofa', 'lamp', 'cabinet', 'bed', 'desk'];
  if (largeKeywords.some(keyword => name.includes(keyword))) {
    return { category: 'large', confidence: 0.8, reason: `filename contains large product keyword` };
  }
  
  // 中型产品关键词
  const mediumKeywords = ['bag', 'shoe', 'boot', 'tablet', 'book', 'handbag', 'backpack', 'clothing'];
  if (mediumKeywords.some(keyword => name.includes(keyword))) {
    return { category: 'medium', confidence: 0.7, reason: `filename contains medium product keyword` };
  }
 ...
 * 基于图像尺寸分析
 */
function analyzeDimensions(metadata: ImageMetadata): { category?: 'small' | 'medium' | 'large'; confidence: number; reason?: string } {
  const { width, height, aspectRatio } = metadata;
  
  // 非常小的图像可能是小产品的特写
  if (width < 800 && height < 800) {
    return { category: 'small', confidence: 0.4, reason: `small image dimensions (${width}x${height})` };
  }
  
  // 超高分辨率可能是大型产品
  if (width > 2000 || height > 2000) {
    return { category: 'large', confidence: 0.3, reason: `high resolution suggests detailed large product` };
  }
  
  // 极端宽高比分析
  if (aspectRatio > 2.0) {
    return { category: 'medium', confidence: 0.3, reason: `wide aspect ratio suggests medium-length product` };
  }
  
  if (aspectRatio < 0.5) {
    return { category: 'small', confidence: 0.4, reason: `tall narrow ratio suggests bottle-like small product` };
  }
 ...
 * 分析用户上下文
 */
function analyzeUserContext(context: string): { category?: 'small' | 'medium' | 'large'; confidence: number; reason?: string } {
  ...
  if (text.match(/\b(small|tiny|mini|compact|delicate|pocket|handheld)\b/)) {
    return { category: 'small', confidence: 0.9, reason: 'user described as small/compact' };
  }
  
  // 大型产品描述
  if (text.match(/\b(large|big|huge|substantial|massive|oversized)\b/)) {
    return { category: 'large', confidence: 0.9, reason: 'user described as large/substantial' };
  }
  
  // 中型产品描述
  if (text.match(/\b(medium|standard|normal|regular|typical)\b/)) {
    return { category: 'medium', confidence: 0.8, reason: 'user described as medium/standard' };
  }
 ...
 * 基于图像方向分析
 */
function analyzeOrientation(orientation: 'portrait' | 'landscape' | 'square'): { category?: 'small' | 'medium' | 'large'; confidence: number; reason?: string } {
  switch (orientation) {
    case 'portrait':
      return { category: 'small', confidence: 0.3, reason: 'portrait orientation suggests bottle/vertical small product' };
    case 'landscape':
      return { category: 'medium', confidence: 0.2, reason: 'landscape orientation suggests horizontal medium product' };
    case 'square':
      return { category: 'small', confidence: 0.2, reason: 'square format often used for small product photography' };
    default:
      return { confidence: 0 };
  }
}
 ...
 * 生成基于启发式分析的提示词
 */
function generateHeuristicHints(category: 'small' | 'medium' | 'large', metadata: ImageMetadata): string[] {
  const hints: string[] = [];
...
export function getSmartSceneRecommendations(analysis: FreeProductAnalysis): {
  ...
  switch (analysis.category) {
    case 'small':
      ...
      reasons.push('Small products work well in intimate, detailed presentations');
      break;
    case 'medium':
