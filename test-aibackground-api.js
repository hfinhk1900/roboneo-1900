...
function printTestResult(testName, response, expectedStatus = 200) {
  ...
  if (response.status === expectedStatus) {
    ...
    console.log(
      `❌ 状态码错误，期望 ${expectedStatus}，实际 ${response.status}`
    );
  }
...
async function runTests() {
  ...
  // 检查认证配置
  if (
    CONFIG.authCookie === 'better-auth.session_token=YOUR_SESSION_TOKEN_HERE'
  ) {
    console.log('\n⚠️  警告: 请先配置认证 Cookie');
  ...
  try {
    ...
    if (
      getResponse.data.backgroundStyles &&
      getResponse.data.backgroundStyles.length > 0
    ) {
    ...
    // 测试 2: POST 请求 - 纯色背景模式
    console.log('\n🔍 开始测试 2: 纯色背景模式...');
    const colorResponse = await sendRequest(
      `${CONFIG.baseUrl}${CONFIG.apiEndpoint}`,
      {
        method: 'POST',
        body: JSON.stringify({
    ...
    if (firstBackgroundStyle) {
      ...
      const aiBackgroundResponse = await sendRequest(
        `${CONFIG.baseUrl}${CONFIG.apiEndpoint}`,
        {
          method: 'POST',
          body: JSON.stringify({
      ...
      printTestResult(
        `AI背景样式 - ${firstBackgroundStyle.name}`,
    ...
    console.log('\n🔍 开始测试 4: 自定义背景样式...');
    const customBackgroundResponse = await sendRequest(
      `${CONFIG.baseUrl}${CONFIG.apiEndpoint}`,
      {
        method: 'POST',
        body: JSON.stringify({
          ...
          customBackgroundDescription:
            'beautiful sunset beach scene with palm trees, tropical paradise background, warm golden hour lighting',
          quality: 'standard',
          steps: 30,
          size: '1024x1024',
          output_format: 'png',
    ...
    // 测试 5: POST 请求 - 透明背景
    console.log('\n🔍 开始测试 5: 透明背景...');
    const transparentResponse = await sendRequest(
      `${CONFIG.baseUrl}${CONFIG.apiEndpoint}`,
      {
        method: 'POST',
        body: JSON.stringify({
